...
import { Appointment, AppointmentStatus, Doctor, Patient } from '../../core/models/hospital.models';
...
@Component({
  ...
  styles: [`
    ...
    .btn-action.complete { background: #dcfce7; color: #15803d; }
...
export class AppointmentsComponent implements OnInit {
  ...
  applyFilter(): void {
    ...
    if (filter === 'All') {
      ...
      this.filteredAppointments.set(this.appointments().filter(a => a.statusName === filter));
    }
  }
  ...
  complete(id: number): void {
    this.hospitalService.completeAppointment(id).subscribe({
      next: (res: any) => {
        this.triggerAlert('success', `Appointment #${id} marked as completed.`);
        this.loadData();
      },
      error: (err: any) => {
        this.triggerAlert('error', err.error?.message || 'Failed to complete appointment.');
      }
    });
  }

  cancel(id: number): void {
    this.hospitalService.cancelAppointment(id).subscribe({
      next: (res: any) => {
        this.triggerAlert('success', `Appointment #${id} has been cancelled.`);
        this.loadData();
      },
      error: (err: any) => {
        this.triggerAlert('error', err.error?.message || 'Failed to cancel appointment.');
      }
    });
  }
  ...
  onSubmit(): void {
    ...
    this.hospitalService.createAppointment({
      patientId: Number(formVal.patientId),
      doctorId: Number(formVal.doctorId),
      appointmentDate: new Date(formVal.appointmentDate!).toISOString(),
      notes: formVal.notes || undefined
    }).subscribe({
      next: (res: any) => {
        this.isSubmitting.set(false);
        this.closeModal();
        this.triggerAlert('success', `Appointment successfully scheduled with ${res.data.doctorName}.`);
        this.loadData();
      },
      error: (err: any) => {
        this.isSubmitting.set(false);
        this.triggerAlert('error', err.error?.message || 'Failed to schedule appointment.');
      }
    });
  }

  private triggerAlert(type: 'success' | 'error', msg: string): void {
